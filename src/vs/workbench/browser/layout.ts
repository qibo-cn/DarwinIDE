...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	getMaximumEditorDimensions(): Dimension {
		...
		const takenHeight =
			...
			(this.isVisible(Parts.ACTIVITYBAR_PART) ? this.activityBarPartView.minimumHeight : 0) +
	...
	private createGridDescriptor(): ISerializedGrid {
		...
		const activityBarHeight = this.activityBarPartView.minimumHeight;
		...
		*/
		...
		const middleSection: ISerializedNode[] = this.state.sideBar.position === Position.LEFT
			...
			? [sideBarNode, ...editorSectionNode]
			: [...editorSectionNode, sideBarNode];
			...
			root: {
				...
				data: [
					...
					
					{
						type: 'leaf',
						data: { type: Parts.ACTIVITYBAR_PART },
						size: activityBarHeight,
						visible: this.isVisible(Parts.ACTIVITYBAR_PART)
					},
